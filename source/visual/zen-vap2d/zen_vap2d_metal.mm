...
namespace Zen { namespace Vap2d {
	...
	void Sprite::_drawSprite()
	{
		...
		auto pipe = Metal::SamplerPipe::GetShared
		(m_is_grey_mode, m_texture->format() == ePixel::Grey,
		 blends.first, blends.second);
		
		r->activePipe(pipe->getID().get());
		
		r->bindTexture(pipe->TextureIndex, m_texture->buffer().v.getID());
		r->setVertexBuffer(pipe->VertexIndex, m_gpus.v.getID().get(), 0);
		r->setVertexBuffer(pipe->VertexExtraIndex, m_gpus.u.getID().get(), 0);
	...
	void Particle::_drawParticle()
	{
		...
		auto pipe = Metal::ParticlePipe::GetShared
		(m_is_grey_mode, m_texture->format() == ePixel::Grey,
		 blends.first, blends.second);
		...
		r->drawPrimitives(Metal::eVertexMode::Point, 0, m_gpus.v_size);
	}
	...
	void Doodle::_drawDoodle()
	{
		...
		auto pipe = Metal::ColorPipe::GetShared
